...
I submit the first version and find that got a TLE, and find that when doing aggregation from either `jump` to `ans`, or from `ans` to `jump`, the complexity is `N*N`, and this can actually be done by prefix sum, and I modify the code and submit and pass! (after this submission, the time is about 1~2 minutes left 😂)	 
